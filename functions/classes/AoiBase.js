//Changed the variable function (line 148)
...
class BaseClient extends Client {
     ...
     */
    async variables(d) {
        while (this.db.ping === undefined || this.db.ping < 0) {
        ...
        if (!this.db?.tables || this.db.tables.length === 0) {
            throw new Error("No tables found in the database.");
        }

        for (const currentTable of this.db.tables) {
            if (currentTable === '__aoijs_vars__') {
                continue;
            }

            for (const [name, value] of Object.entries(d)) {
                this.variableManager.add({ name, value, table: currentTable });
            }
        }
    }
